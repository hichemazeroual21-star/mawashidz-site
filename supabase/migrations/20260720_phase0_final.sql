-- ============================================================
-- MawashiDZ — Phase 0 FINAL (production baseline)
-- Canonical migration · idempotent · safe to re-run · no data loss
-- Supersedes all prior Phase 0 migrations (see migrations/archive/)
-- Version: Phase 0 Final · 2026-07-20
-- ============================================================
...
returns text language sql immutable set search_path = public as $$
  select case lower(trim(coalesce($1, '')))
    when 'buyer' then 'buyer' when 'breeder' then 'breeder'
    when 'vet' then 'vet' when 'feed' then 'feed' else 'buyer' end;
$$;
...
declare clean text := nullif(trim(coalesce(date_input, '')), '');
begin
  if clean is null or clean !~ '^\d{4}-\d{2}-\d{2}$' then return null; end if;
  return clean::date;
...
declare role_prefix text; next_value bigint; normalized_role text;
begin
...
declare normalized_role text; assigned_member_id text;
begin
  ...
  new.raw_user_meta_data := coalesce(new.raw_user_meta_data, '{}'::jsonb)
    || jsonb_build_object('member_id', assigned_member_id, 'role', normalized_role, 'status', 'pending');
  return new;
end; $$;

create or replace function public.handle_new_user()
returns trigger language plpgsql security definer set search_path = public as $$
declare user_role text; assigned_member_id text; assigned_birth_date date;
begin
...
-- ------------------------------------------------------------
-- 4) Grants (explicit, no duplicates)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 7) Triggers (single active trigger per purpose)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 8) Backfill legacy rows + sync counters
-- ------------------------------------------------------------
do $$
